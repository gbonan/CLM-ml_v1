  ...
  subroutine SolarRadiation (bounds, num_filter, filter, mlcanopy_inst)
    ...
    do fp = 1, num_filter
       ...
       do ic = ntop(p), nbot(p) , -1
          ...
          chil = min(max(xl(patch%itype(p)), chil_min), chil_max)
    ...
    do ib = 1, numrad
       do fp = 1, num_filter
          ...
          do ic = nbot(p), ntop(p)
             ...
             s1 = exp(-h * clump_fac(patch%itype(p)) * dpai(p,ic))
             s2 = exp(-kb(p,ic) * clump_fac(patch%itype(p)) * dpai(p,ic))
             ...
             iabsb_sun(p,ic,ib) = (1._r8-omega(p,ic,ib)) * ((1._r8-s2) * unitb + clump_fac(patch%itype(p)) / avmu(p,ic) * (a1b+a2b))
             ...
             ! sun = sun + 0.5_r8 * omega(p,ic,ib) * clump_fac(patch%itype(p))* (1._r8-s2*s2) * unitb * tbi(p,ic)
             ! sun = sun + 0.5_r8 * clump_fac(patch%itype(p)) * g1 * (1._r8-s2*s2) * tbi(p,ic)
             ! sun = sun + 0.5_r8 * clump_fac(patch%itype(p)) * g2 * (1._r8-s2*s2) * tbi(p,ic)
             ...
             iabsd_sun(p,ic,ib) = (1._r8 - omega(p,ic,ib)) * clump_fac(patch%itype(p)) / avmu(p,ic) * (a1d + a2d)
          ...
          do ic = ntop(p), nbot(p), -1
             ...
             dir = dir * exp(-kb(p,ic) * clump_fac(patch%itype(p)) * dpai(p,ic))
